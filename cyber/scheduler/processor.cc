...
void Processor::Run() {
	...
	while (cyber_likely(running_.load())) {
		if (cyber_likely(context_ != nullptr)) {
			auto croutine = context_->NextRoutine(); //lubin - to get next task 
			if (croutine) {
				...
				//lubin - if no croutine wait here to be notified by task
				snap_shot_->execute_start_time.store(0);
				context_->Wait();
			}
		} else {  //lubin - if stop processor
...
void Processor::BindContext(const std::shared_ptr<ProcessorContext>& context) {
	context_ = context;
	std::call_once(thread_flag_, [this]() { thread_ = std::thread(&Processor::Run, this); }); //lubin - thread safe
}
