...
bool Player::Start() {
	...
	std::cout << std::fixed;
	const double total_progress_time_s = static_cast<double>(play_param.end_time_ns - play_param.begin_time_ns) / 1e9 + static_cast<double>(play_param.start_time_s);
	...
	while (!is_stopped_.load() && apollo::cyber::OK()) {
		...
		if (consumer_->last_played_msg_real_time_ns() > 0) {
			...
			consumer_->base_msg_play_time_ns() + consumer_->base_msg_real_time_ns() -
			producer_->play_param().begin_time_ns) /1e9;
		}
